...
public class PSFAnalyzer__ implements PlugIn, Measurements {
	...
	static JComboBox comboFit;
	static JCheckBox checkPS, checkManual;
	...
	public void createAndShowGUI() {
		...
		comboFit = new JComboBox<String>();
		String[] comboItems = new String[] { "Gaussian Fit", "Poisson Fit", "Lorentzian Fit" };
		for (int i = 0; i < comboItems.length; i++)
			comboFit.addItem(comboItems[i]);
		...
		checkManual = new JCheckBox("Manual Selection");
	...
	public void processImage() {
		Thread process0 = new Thread(new Runnable() {
			...
			public void run() {
				...
				if (checkManual.isSelected() == Boolean.TRUE) {
				...
				if (checkManual.isSelected() == Boolean.FALSE) {
